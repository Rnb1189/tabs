...
export default class SwipeableTabBarNode extends React.Component {
  constructor(props) {
    ...
    const { hasPrevPage, hasNextPage } = this.checkPaginationByKey(
  ...
  onPan = e => {
    ...
    const { hasPrevPage, hasNextPage } = this.checkPaginationByDelta(
      this.cache.totalDelta
    );
    if (
